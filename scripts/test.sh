...
start_ganachecli() {
  ...
  local accounts=(
    ...
    --account="0x2bdd21761a483f71054e14f5b827213567971c676928d9a1808cbfa4b7501210,100000000000000000000000000"
  )

  if [ "$SOLIDITY_COVERAGE" = true ]; then
    node_modules/.bin/ganache-cli --gasLimit 0xfffffffffff --port "$ganachecli_port" "${accounts[@]}" > /dev/null &
  else
    node_modules/.bin/ganache-cli --gasLimit 0xfffffffffff --port "$ganachecli_port" "${accounts[@]}" >> "$HOME/ganache_cli.log" 2>&1 &
  fi
